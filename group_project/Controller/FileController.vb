...
Public Class FileController
    ...
    'Takes care of exceptions that could happen in the middle of serialization process, if it happens, this function will return false
    Private Function serialize(ByVal file As File, ByVal filePath As String) As Boolean
        Dim ret As Boolean = False

        If (Not file Is Nothing) And (Not String.IsNullOrEmpty(filePath)) Then 'First checking for null file and empty file path
            If (Not file.BudgetManager Is Nothing) And (Not file.World Is Nothing) And (Not file.PointArray Is Nothing) Then 'This will check if information inside is valid
    ...
    'Takes care of exceptions that could happen in the middle of deserialization process, if it happens, this function will return null
